	...
	jQuery(function() {
		...
		var sticky_navigation = function(){
			...
			if (scroll_top > sticky_navigation_offset_top) {
				...
				jQuery('#sticky_navigation').addClass('shrunk-header');
				jQuery('#sticky_navigation').removeClass('no-shrunk-header');
			} else {
				jQuery('#sticky_navigation').stop(true).animate({ 'padding':'20px 0;','min-height':'60px', 'opacity' : 1}, 100);
				jQuery('#sticky_navigation').css({ 'position': 'relative' });
				jQuery('#sticky_navigation').removeClass('shrunk-header');
				jQuery('#sticky_navigation').addClass('no-shrunk-header');
			}
		};
